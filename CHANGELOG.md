...
- Found while auditing the code: this project's
  ...
  real, plain HTTP (bare `app.listen()`) with zero TLS code anywhere - a
  real gap between a public security claim and the actual behavior.
- **`src/tls.ts`** (new) - `resolveTlsConfig()` reads real, optional env
  vars: `TLS_CERT_PATH`/`TLS_KEY_PATH` switch the gateway to real HTTPS
  (`minVersion: "TLSv1.2"` - TLS 1.0/1.1 are never negotiated); adding
  `TLS_CLIENT_CA_PATH` turns that into real mutual TLS (`requestCert:
  true, rejectUnauthorized: true`), refusing the TLS handshake itself for
  any client without a certificate signed by that CA. Deliberately fails
  loud: a configured-but-unreadable path throws at startup rather than
  silently falling back to plaintext HTTP - only the complete absence of
  both TLS_CERT_PATH and TLS_KEY_PATH means "TLS not configured yet".
- **`src/server.ts`** - `main()` now calls `resolveTlsConfig()` and
  creates `https.createServer(tlsConfig, app)` instead of a bare
  `app.listen()` when TLS is configured; the startup banner gained a
  `SECURITY:` line reporting which of the three real modes (plain HTTP /
  TLS / mutual TLS) is active.
- **`tests/tls.test.ts`** (new, 9 tests) - real X.509 material generated
  fresh via the real `openssl` binary in a temp directory (never a
  committed fixture): config-resolution edge cases, plus a genuine
  end-to-end mTLS handshake test against a real `https.Server` - proving
  a client presenting a CA-signed certificate connects successfully, and
  a client presenting none is refused at the TLS handshake itself, not a
  later application-level check.
- **`.env.example`**/**`docs/API.md`** (new "Security: TLS / mutual TLS"
  section)/README (all 7 languages) - badge corrected from the unpinned
  "TLS 1.3" claim to the real "TLS 1.2+" floor this implementation
  actually enforces; the Key Features bullet now describes the real,
  opt-in, gateway-only scope instead of implying mTLS already covers
  "all factory connections" (the three protocol children are still
  reachability-probed over their own native transports, unchanged).
- Verified: `npm run typecheck`, full `npm test` (42/42, including the
  new suite), `npm run build`.
